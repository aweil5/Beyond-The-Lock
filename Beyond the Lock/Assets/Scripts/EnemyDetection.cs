...
public class EnemyDetection : MonoBehaviour
{
    ...
    /*private void DetectPlayer()
    {
        ...
        //Checking if the player is in the detection range
        if (distanceToPlayer <= detectionRange)
        {
            ...
            {
                //Make sure there isn't an object (Ex: Pillar) in between the enemy and player
                if (!Physics.Raycast(transform.position, directionToPlayer, distanceToPlayer, obstructionMask))
                {
                    // Player is in sight and not objects in way
                    playerInSight = true;
                    OnPlayerDetected();
                }
                else
                {
                    //If there is object in way 
                    playerInSight = false;
                }
            }
            else
            {
                //Player not in the site angle
                playerInSight = false;
            }
        }
        else
